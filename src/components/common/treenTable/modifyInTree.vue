    ...
    <el-table
      ...
      <el-table-column label="操作" v-if="treeType === 'normal'" width="150"  fixed="right">
        <template scope="scope">
          ...
          <el-button size="small" type="warning" @click="amendForm(scope.$index, scope.row)">更改</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 修改弹出框-->
      ...
      <div class="amend">
        <el-dialog
          ...
          <el-form :inline="true" :label-position="labelPosition" label-width="80px" :model="amendFormLabel">
            <el-form-item v-for="(list,index) in columns" :key="list.dataIndex" :label="list.text" :prop="amendFormLabel[list.dataIndex]">
